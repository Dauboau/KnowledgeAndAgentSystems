...
extensions [table rnd] ;extend this, if you want
...
TO GO
  if chooseMyParams [
    set producersProduction 1.6
    set stockDecreaseRetailer 0.6

    set numberTraders 35
    set saleQuantity 15

    set chooseTheBest false
  ]
...
to chooseProduct
  ifelse chooseTheBest [
    ...
    ; Calculate expected profit for each product and store it
    foreach products [ [prd] ->
      let profit table:get estimatedSellPrice prd - table:get estimatedBuyPrice prd
    ...
    let weights []
    let total_sum sum profits

    foreach profits  [ [prf] -> 
      let weight prf / total_sum
      set weights lput weight weights
    ]

    let pairs (map list products weights)
    set product first rnd:weighted-one-of-list pairs [[p] -> last p]
  ]
