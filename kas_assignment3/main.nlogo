...
;; And adapted from code for the tournament created by Harmen de Weerd.
...
globals [
  ...
  ;; the actions a turtle can take: cooperate or defect
  COOPERATE
  DEFECT
]
...
turtles-own [
  score
  strategy
  action              ;; action of turtle: cooperate/defect
  partner-defected?   ;; action of the partner
  partnered?          ;; am I partnered?
  partner             ;; WHO of my partner (nobody if not partnered)
  all-play-history    ;; your own history against each turtle (indexed by who values)
  all-play-partner-history   ;; a list containing the actions of other turtles
                      ;; against you (indexed by WHO values)
                      ;; These are of the format [[][][]...[][]],
                      ;; where each list contains the interactions with one turtle
]
...
to setup
  ...
  ;; COOPERATE is set to false and DEFECT to true
  set COOPERATE false
  set DEFECT true
  reset-ticks
end
...
to make-turtles
  ...
  create-turtles num-grim [set strategy "grim" set color magenta ]
  create-turtles num-my-strat [ set strategy "my-strat" set color turquoise - 1 ]
end
...
to setup-common-variables
  ...
  setup-history-lists    ;; Initialize the history lists for all turtles
end

;; Initialize history lists in all turtles
to setup-history-lists
  let num-turtles count turtles

  ;; initialise each turtle's own and partner history list.
  ask turtles [
    set all-play-history []
    set all-play-partner-history []
    while [length all-play-history <= num-turtles] [
      set all-play-history lput [] all-play-history
      set all-play-partner-history lput [] all-play-partner-history
    ]
  ]
end
...
to go
  ...
  ask partnered-turtles [ play-a-round ([who] of partner)]
  do-scoring
  tick
end
...
to select-action    ;; turtle procedure
  set action COOPERATE    ;; set action to COOPERATE as a fail safe
  ;; play-partner-history is the actions of the partner against this turtle
  let play-partner-history item ([who] of partner) all-play-partner-history
  ;; play-history is the actions of the turtle against this partner
  let play-history item ([who] of partner) all-play-history
  if strategy = "random" [ act-randomly play-history play-partner-history]
  if strategy = "cooperate" [ cooperate-strat play-history play-partner-history]
  if strategy = "defect" [ defect-strat play-history play-partner-history]
  if strategy = "tit-for-tat" [ tit-for-tat play-history play-partner-history]
  if strategy = "grim" [ grim play-history play-partner-history]
  if strategy = "my-strat" [ my-strat play-history play-partner-history]
end

to play-a-round [turtle2]    ;; turtle procedure
  get-payoff     ;; calculate the payoff for this round
  update-history turtle2    ;; store the results for next time
end

;; Calculate the payoff for this round and display a label with that payoff.
to get-payoff
  set partner-defected? [action] of partner
  ifelse partner-defected? [
    ifelse action [
      set score (score + Punishment) set label Punishment
    ] [
      set score (score + Sucker) set label Sucker
    ]
  ] [
    ifelse action [
      set score (score + Temptation) set label Temptation
    ] [
      set score (score + Reward) set label Reward
    ]
  ]
end

;; Update the history of turtle by adding its current action.
;; Also keeps track of what the turtle's partner did by adding its current action.
to update-history [turtle2]
  set all-play-history replace-item turtle2 all-play-history lput action item turtle2 all-play-history
  set all-play-partner-history replace-item turtle2 all-play-partner-history lput partner-defected? item turtle2 all-play-partner-history
end
...
to cooperate-strat [play-history play-partner-history]
...
to tit-for-tat [play-history play-partner-history]
  ...
  ifelse (empty? play-partner-history) [
    ...
    set action item (length play-partner-history - 1) play-partner-history  ;; mirror partner's last action
  ]
end
...
to grim [play-history play-partner-history]
  ...
  ifelse member? DEFECT play-partner-history [
    set action DEFECT    ;; defect forever if partner has defected
  ] [
    set action COOPERATE    ;; otherwise, cooperate
  ]
end
...
to do-scoring
  ...
  set grim-score (calc-score "grim" num-grim)
...
"my-strat" 1.0 0 -14835848 true "" "if num-my-strat-games > 0 [ plot my-strat-score / (num-my-strat-games) ]"
"grim" 1.0 0 -5825686 true "" "if num-grim-games > 0 [ plot grim-score / (num-grim-games) ]"
...
495
average-random
ifelse-value (num-random-games > 0) [random-score / num-random-games][0]
3
1
11
...
495
average-cooperate
ifelse-value (num-cooperate-games > 0) [cooperate-score / num-cooperate-games][0]
3
1
11
...
495
average-defect
ifelse-value (num-defect-games > 0) [defect-score / num-defect-games][0]
3
1
11
...
546
average-strat-one
ifelse-value (num-my-strat-games > 0) [my-strat-score / num-my-strat-games][0]
3
1
11
...
546
average-tit-for-tat
ifelse-value (num-tit-for-tat-games > 0) [tit-for-tat-score / num-tit-for-tat-games][0]
3
1
11
...
546
average-grim
ifelse-value (num-grim-games > 0) [grim-score / num-grim-games][0]
3
1
11
